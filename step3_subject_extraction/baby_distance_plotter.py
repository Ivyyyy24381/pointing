"""
Baby Distance to Targets Plotter
Generates time-series plot showing Euclidean distance from baby to each target over time.
"""
...
class BabyDistancePlotter:
    """Creates distance vs time plots for baby-to-target distances."""
    ...
    def create_distance_plot(self, csv_path: Path, output_image_path: Path,
                            title: str = "Baby Distance to Targets Over Time"):
        """
        Create a plot showing Euclidean distance from baby to each target over time.

        Args:
            csv_path: Path to processed_baby_result_table.csv
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        print("Usage: python baby_distance_plotter.py <csv_path> [output_path]")
        sys.exit(1)
    ...
    if len(sys.argv) > 2:
        ...
        output_path = csv_path.parent / "baby_distance_to_targets.png"
