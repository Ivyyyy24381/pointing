...
class SkeletonExtractorUI:
    ...
    def setup_controls(self, parent):
        ...
        ttk.Label(settings_frame, text="Subject Type:").grid(row=9, column=0, sticky="w")
        self.subject_type_var = tk.StringVar(value="human")
        ttk.Radiobutton(settings_frame, text="👤 Human", variable=self.subject_type_var,
                       value="human", command=self.on_subject_type_change).grid(row=10, column=0, sticky="w")
        ttk.Radiobutton(settings_frame, text="🐶 Dog", variable=self.subject_type_var,
                       value="dog", command=self.on_subject_type_change).grid(row=11, column=0, sticky="w")
        ttk.Radiobutton(settings_frame, text="👶 Baby", variable=self.subject_type_var,
                       value="baby", command=self.on_subject_type_change).grid(row=12, column=0, sticky="w")
    ...
    def on_subject_type_change(self):
        """Handle subject type change (Human/Dog/Baby)."""
        subject_type = self.subject_type_var.get()
        print(f"\n🔄 Subject type changed to: {subject_type.upper()}")

        if subject_type == "human":
            # Human detection using MediaPipe Pose
            print(f"   Using MediaPipe Pose for human detection")
            messagebox.showinfo("Subject Type", "Human detection selected.\n\nUsing MediaPipe Pose for skeleton extraction.")
        elif subject_type == "dog":
            # Dog detection using DeepLabCut
            print(f"   Dog detection requires DeepLabCut superanimal model")
            messagebox.showinfo("Subject Type", "Dog detection selected.\n\n⚠️ Dog detection requires:\n"
                              "1. DeepLabCut installed\n"
                              "2. Segmented video (masked_video.mp4)\n"
                              "3. SuperAnimal quadruped model\n\n"
                              "This feature is coming soon!")
            # TODO: Implement dog detection
        elif subject_type == "baby":
            # Baby detection using MediaPipe Pose (same as human)
            print(f"   Using MediaPipe Pose for baby detection")
            messagebox.showinfo("Subject Type", "Baby detection selected.\n\nUsing MediaPipe Pose (same as human detection).\n"
                              "Note: Requires good visibility of baby's body.")

        # Reinitialize detector with new subject type
        # For now, only human/baby work with MediaPipe
        if subject_type in ["human", "baby"]:
            self.initialize_detector()
            if self.current_color is not None:
                self.process_current_frame()
